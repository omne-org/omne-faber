...
7. **approve** (`command: synthesize-review`, `gate: human-approval`) — re-runs the same synthesize skill as a recap, then runs the `human-approval` gate hook. The gate resolves to `dist/hooks/human-approval.{ps1|sh}` on the current platform (exit 0 → `gate.passed`, non-zero → `node.failed{gate_failed}`, 60s timeout → `node.failed{gate_timeout}`).
...
**`approve` reuses `synthesize-review`.** The last node carries `command: synthesize-review` with a `gate: human-approval` modifier, not a standalone `prompt:` stub. The plan flagged this choice: a stub prompt node would satisfy the schema but exists only to hang a gate off. Re-running the synthesize skill produces a fresh recap of the merged review at the moment a human is asked to sign off, which is genuinely useful; it costs one extra AI call in exchange for avoiding a no-op node the validator would still have to special-case. If the gate hook exits quickly the redundant call is cheap, and if a human keeps the gate open for hours the recap is closer to the real decision point than the earlier `synthesize` output would be.
